...
static int curr_lines_index = 0;
static float *xlinecoords;
static float *ylinecoords;
//
...
//create a array to store the coordinates of
    ...
    xlinecoords = (int*)calloc(coords_size/10, sizeof(int));
    ylinecoords = (int*)calloc(coords_size/10, sizeof(int));
    //
...
//prevx and prevy store the previous location of the tracked object
    ...
    while(!demo_done){
        ...
        if (count > 1){
          ...
          time_ ++;
        //Compare s a boolean that shows whether there is a previous frame. If not compare = 0, and we only need to update the current location
          if (compare == 0){
            ...
            if (curr_coords_index % 10 == 0) {
                xlinecoords[curr_lines_index] = prevx;
                ylinecoords[curr_lines_index] = prevy;
                curr_lines_index ++;
            }
            curr_coords_index ++;
            //
        //Then we update compare to 1
          ...
          if (((fabs(currw - prevw) >  fmin(prevw, currw) || fabs(currh-  prevh) >  fmin(prevh, currh)) || fabs(currx-prevx) > currw || fabs(curry - prevy) > currh || strcmp(currname, prevname) != 0 || (int)(movement[0]) == 0) && countframe < 29){
            printf("loss!!!!!!!!!!!!!!!!!!!!!! \n");
            printf("dx is %f \n", dx);
            printf("dy is %f \n", dy);
            // currx = prevx+dx;
            ...
            printf("prediction x is %f \n", currx);
            printf("prediction y is %f \n", curry);
        ...
        float b = currx - prevx;
        float k = currx - b*time_;
        pred_x = (time_ + 1)*b + k;

        float b_y = curry - prevy;
        float k_y = curry - b_y*time_;
        pred_y = (time_ + 1)*b_y + k_y;
        ...
        //Kyle 7/6/2017
          draw_tracers(buff[(buff_index + 1)%3], curr_coords_index, xcoords, ycoords, curr_lines_index, xlinecoords, ylinecoords);


        }
        draw_box_width(buff[(buff_index + 1)%3], (int)((pred_x-currw/2)*buff[(buff_index + 1)%3].w), (int)((pred_y-currh/2)*buff[(buff_index + 1)%3].h),(int)((pred_x+currw/2)*buff[(buff_index + 1)%3].w), (int)((pred_y+currh/2)*buff[(buff_index + 1)%3].h), buff[(buff_index + 1)%3].w/80, 0.0, 1.0, 0.0);
        //
      }
        if(!prefix){
